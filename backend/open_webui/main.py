...
async def lifespan(app: FastAPI):
    ...
    start_logger()
    # --- DB hotfix: ensure folder table has context columns (older DBs may miss these)
    try:
        import os, sqlite3
        from open_webui.env import DATABASE_URL

        if DATABASE_URL.startswith('sqlite'):
            # DATABASE_URL like: sqlite:////app/backend/data/webui.db
            ...
            if os.path.exists(db_path):
                ...
                # Backfill to 'general' for older rows
                cur.execute("""
                    UPDATE folder
                ...
                if altered:
                    log.info('Applied folder table schema hotfix (context_type/course_id/lab_id)')
    except Exception as e:
        # Don't block startup; just avoid crashing on missing columns
        log.warning(f'Folder table schema hotfix failed: {e}')
