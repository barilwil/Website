...
async def get_ollama_url(request: Request, model: str, url_idx: Optional[int] = None):
    """Resolve which Ollama base URL to use for a given model.

    Historically Open WebUI defaults to localhost/host.docker.internal. When users add an
    authenticated Ollama Cloud endpoint (https://ollama.com) alongside a local endpoint,
    we should prefer the endpoint that matches the model's remote_host (if provided).
    """

    base_urls: list[str] = request.app.state.config.OLLAMA_BASE_URLS
    models = request.app.state.OLLAMA_MODELS
    ...
    if url_idx is None:
        ...
        model_entry = models[model] or {}
        candidate_idxs = list(model_entry.get('urls', []))

        # Prefer an enabled URL that matches the model's remote_host (common for cloud models)
        remote_host = model_entry.get('remote_host')
        if not remote_host and isinstance(model_entry.get('details'), dict):
            remote_host = model_entry['details'].get('remote_host')

        def _norm_netloc(n: str) -> str:
            return n[:-4] if n.endswith(':443') else n
        ...
        if remote_host:
            try:
                ...
                if '://' not in rh:
                    rh = 'https://' + rh
                pr = urlparse(rh)
                remote_netloc = _norm_netloc(pr.netloc)

                for idx, u in enumerate(base_urls):
                    pu = urlparse(u)
                    if _norm_netloc(pu.netloc) == remote_netloc:
                        api_cfg = request.app.state.config.OLLAMA_API_CONFIGS.get(
                            str(idx),
                            request.app.state.config.OLLAMA_API_CONFIGS.get(u, {}),
                        )
                        if api_cfg.get('enable', True):
                            preferred_idx = idx
                            break
            except Exception:
                preferred_idx = None

        # If model came from multiple backends, prefer external connections when available
        if preferred_idx is None and candidate_idxs:
            for idx in candidate_idxs:
                ...
                if api_cfg.get('enable', True) and api_cfg.get('connection_type') == 'external':
                    preferred_idx = idx
                    break

        if preferred_idx is not None:
            url_idx = preferred_idx
        else:
            if not candidate_idxs:
                # Fallback to first configured URL
                url_idx = 0
            else:
                url_idx = random.choice(candidate_idxs)

    # Bound check
    if url_idx < 0 or url_idx >= len(base_urls):
        raise HTTPException(status_code=500, detail='Invalid Ollama URL index')
