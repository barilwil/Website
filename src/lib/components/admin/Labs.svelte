							...
							>
								<div class="min-w-0 flex-1">
									<div class="font-medium truncate">
										{course.code}
										<span class="ml-1 text-xs text-neutral-500 truncate">{course.name}</span>
									</div>
								...
								<div class="flex items-center gap-2 flex-shrink-0 whitespace-nowrap pl-3">
									<label class="flex items-center gap-1 text-[11px] text-neutral-500 whitespace-nowrap">
										<input
											type="checkbox"
									...
									<button
										type="button"
										class="text-[11px] text-red-500 hover:text-red-600 underline whitespace-nowrap"
										on:click={(event) => {
											event.stopPropagation();
						...
						{:else}
							<div class="space-y-1 max-h-64 overflow-y-auto">
								{#each selectedCourse.labs as lab}
									...
									>
										<div class="min-w-0 flex-1">
											<div class="font-medium truncate">{lab.name}</div>
											{#if lab.description}
												<p class="text-xs text-neutral-500 line-clamp-2 mt-0.5">
													{lab.description}
												</p>
										...
										<div class="flex items-center gap-3 flex-shrink-0 whitespace-nowrap pl-3">
											<label class="flex items-center gap-1 text-[11px] text-neutral-500 whitespace-nowrap">
												<input
													type="checkbox"
													checked={lab.enabled}
