<script lang="ts">
	...
	onMount(async () => {
		...
		// In the message input, only show knowledge bases that are actually usable as attachments.
		// (Empty knowledge bases often come from old/deleted lab setups and confuse users.)
		let collections = $knowledge
			.filter((item) => !item?.meta?.document)
			.filter((item) => {
				const filesLen = (item?.files ?? []).length;
				const fileIdsLen = (item?.data?.file_ids ?? []).length;
				return filesLen > 0 || fileIdsLen > 0;
			})
			.map((item) => ({
				...item,
				type: 'collection'
